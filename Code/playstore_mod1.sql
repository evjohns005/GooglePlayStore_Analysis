# 1. Which apps have the highest rating in the given available dataset?

SELECT App, Rating 
FROM playstore_data.cleaned_apps 
WHERE Rating = (SELECT max(Rating) FROM playstore_data.cleaned_apps);
# Query shows top based on rating of 5.0 which is the highest, but results stay the same as 'Hojiboy Tojiboyev Life Hacks' being top rated.

# 2. What are the number of installs and reviews for the above apps? Return the apps with the highest reviews to the top.

SELECT App, Rating, Reviews, Installs 
FROM playstore_data.cleaned_apps 
WHERE Rating = (SELECT max(Rating) FROM playstore_data.cleaned_apps) 
ORDER BY Reviews DESC;
#Highly rated App that has the most reviews= 'Rios de Fe'

# 3. Which app has the highest number of reviews? Also, mention the number of reviews and category of the app

SELECT App, Reviews, Category
FROM playstore_data.cleaned_apps 
WHERE Reviews = (SELECT max(Reviews) FROM playstore_data.cleaned_apps);
#Highest number of reviews received shows the 'Facebook' at 78,158,306

# 4. What is the total amount of revenue generated by the google play store by hosting apps? 
# (Whenever a user buys apps  from the google play store, the amount is considered in the revenue)
SELECT * FROM playstore_data.cleaned_apps;

SELECT ROUND(SUM(Price), 2)
FROM playstore_data.cleaned_apps;
# Total amount after rounding shows the total amount of revenue to be $10636

# 5. Which Category of google play store apps has the highest number of installs? also, find out the total number of installs for that particular category.
SELECT Category, COUNT(Installs) as Number_Of_Installs
FROM playstore_data.cleaned_apps
GROUP BY Category
ORDER BY COUNT(Installs) DESC;
# Highest number of installs from FAMILY category.


# 6. Which Genre has the most number of published apps?
SELECT * FROM playstore_data.cleaned_apps;

SELECT COUNT(App), Genres
FROM playstore_data.cleaned_apps
GROUP BY Genres
ORDER BY COUNT(App) DESC;
# Looks like the most published app is in the Tools Genre. 

# 7. Provide the list of all games ordered in such a way that the game that has the highest number of installs is displayed on the top
# (to avoid duplicate results use distinct)

SELECT DISTINCT App, Category, Installs
FROM playstore_data.cleaned_apps
WHERE Category = 'GAME'
ORDER BY Installs DESC;
# The game that was installed the highest was 'Subway Surfers'

# 8. Provide the list of apps that can work on android version 4.0.3 and UP.
SELECT App, Android_Version
FROM playstore_data.cleaned_apps
WHERE Android_Version = '4.0.3 and up';

# 9. How many apps from the given data set are free? Also, provide the number of paid apps.
SELECT * FROM playstore_data.cleaned_apps;

SELECT Type, COUNT(Type) as Total
FROM playstore_data.cleaned_apps
GROUP BY Type;
# There are 9580 free apps compared to only 765 paid apps.

# 10. Which is the best dating app? (Best dating app is the one having the highest number of Reviews)
SELECT App, Reviews, Genres
FROM playstore_data.cleaned_apps
WHERE Genres= 'Dating'
ORDER BY Reviews DESC;
# The best dating app is 'Zoosk Dating App: Meet Singles' with 516,917 reviews.

#11. Get the number of reviews having positive sentiment and number of reviews having negative sentiment for the app 10 best foods for you and compare them.
SELECT * FROM playstore_data.cleaned_reviews;

SELECT App, Sentiment, COUNT(Sentiment) as Number_Of_Reviews
FROM playstore_data.cleaned_reviews
WHERE App= '10 Best Foods for You'
GROUP BY Sentiment;
# The app '10 Best Foods for You' has a total of 162 positive reviews with only 10 negative.

# 12. Which comments of ASUS SuperNote have sentiment polarity and sentiment subjectivity both as 1?
SELECT *
FROM playstore_data.cleaned_reviews
WHERE App= 'ASUS SuperNote' AND Sentiment_Polarity=1 AND Sentiment_Subjectivity=1;
# Only one comment pulled up with a sentiment_polarity and sentiment_subjectivity=1. Comment= 'Awesome!!!!'

# 13. Get all the neutral sentiment reviews for the app Abs Training-Burn belly fat 
SELECT *
FROM playstore_data.cleaned_reviews
WHERE App='Abs Training-Burn belly fat' AND Sentiment='Neutral';

# 14. Extract all negative sentiment reviews for Adobe Acrobat Reader with their sentiment polarity and sentiment subjectivity
SELECT DISTINCT Translated_Reviews, App, Sentiment, Sentiment_Polarity, Sentiment_Subjectivity
FROM playstore_data.cleaned_reviews
WHERE App='Adobe Acrobat Reader' AND Sentiment='Negative';



